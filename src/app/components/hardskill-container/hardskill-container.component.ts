...
export class HardskillContainerComponent implements OnInit {
  ...
  listadoHardSkills : any;
  listadoHardSkillsAux : any;

  constructor(private hardSkillsService : HardskillsService, private _snackBar : MatSnackBar) {
  }

  deleteHardSkill(event: any) {
  this.listadoHardSkills = this.listadoHardSkills.filter((item : any)=> item.hardskill_id !== event.hardSkillId);
  this.hardSkillsService.delete(event.hardSkillId)
  .subscribe({
    next: response => {
      this.listadoHardSkillsAux = this.listadoHardSkills;
    openSnackBar(this._snackBar, " HardSkill '" +  event.hardSkillDescription + " 'dropped", 'red-snackbar', "x" );
   },
    error: (error:any) => {
      openSnackBar(this._snackBar, `No se pudo eliminar la habilidad ${event.hardSkillDescription} por ${error.error.error}`, "red-snackbar", "x" );
      this.listadoHardSkills = this.listadoHardSkillsAux;
    }})
}

  ngOnInit(): void {
    this.hardSkillsService.getAll()
    .subscribe({
      next: response => {
