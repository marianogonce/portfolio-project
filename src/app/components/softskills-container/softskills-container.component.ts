...
export class SoftskillsContainerComponent implements OnInit {
  ...
  deletesoftSkill(event: any) {
    this.listadoDeSoftSkills = this.listadoDeSoftSkills.filter(
      (item: any) => item.softskill_Id !== event.id
    );
    this.softskillService.delete(event.id).subscribe({
      next: (response) => {
        this.listadoDeSoftSkillsAuxiliar = this.listadoDeSoftSkills;
        openSnackBar(
          this._snackBar,
          " SoftSkill '" + event.descripcion + " 'dropped",
          'info-snackbar',
          'x'
        );
      },
      error: (error: any) => {
        openSnackBar(
          this._snackBar,
          `No se pudo eliminar la habilidad ${event.descripcion} por ${error?.message}`,
          'red-snackbar',
          'x'
        );
  ...
  ngOnInit(): void {
    this.softskillService.getAll().subscribe({
      next: (response) => {
